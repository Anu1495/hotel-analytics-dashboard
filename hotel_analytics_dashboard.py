    ...
    with st.spinner("Fetching Google Ads performance data..."):
        ...
        if ads_manager.initialize_client():
            ...
            if not ads_data.empty:
                # Calculate totals
                total_spend = ads_data['cost'].sum()
                total_clicks = ads_data['clicks'].sum()
                total_impressions = ads_data['impressions'].sum()
                
                # Get spend breakdown by campaign with proper type identification
                spend_breakdown = ads_data.groupby(['campaign_id', 'campaign_name']).agg({
                    'cost': 'sum',
                    'clicks': 'sum',
                    'impressions': 'sum'
                }).reset_index()
                
                # Add campaign type identification - KEEP Performance Max naming for Google Ads
                def get_campaign_type(name):
                    ...
                    # Then check for Paid Search
                        ...
                        return 'Display'
                    elif 'video' in name_lower or 'youtube' in name_lower:
                        return 'Video'
                    elif 'shopping' in name_lower or 'product' in name_lower:
                        return 'Shopping'
...
def fetch_ga4_paid_revenue_by_source_medium(property_id, start_date, end_date, source_medium):
    ...
    try:
        ...
        # Use sessionSourceMedium for exact matching
        request = RunReportRequest(
            property=f"properties/{property_id}",
            dimensions=[Dimension(name="date")],
            metrics=[
            ...
            dimension_filter=FilterExpression(
                and_group=FilterExpressionList(
                    expressions=[
                        # Filter for specific source/medium
                        FilterExpression(filter=Filter(
                            field_name="sessionSourceMedium",
                            string_filter=Filter.StringFilter(value=source_medium))),
        ...
        st.error(f"Failed to fetch GA4 paid purchase data for {source_medium}: {str(e)}")
        return pd.DataFrame(columns=['date', 'revenue', 'purchases'])
...
def main():
    ...
    # Month selection for overall performance
    current_date = datetime.now()
    available_months = get_available_months()
    selected_month_overall = st.selectbox(
        "Select Month for Overall Performance",
        options=[f"{month} {year}" for month, year in available_months],
        index=0,  # Default to current month
        key="overall_month_selector",
